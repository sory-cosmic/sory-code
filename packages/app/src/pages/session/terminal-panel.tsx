...
import { resolveInitialEnvironment } from "@/components/edit-project"
...
/**
 * Small badge showing which execution environment the terminal workspace
 * runs in. Local projects show nothing; sandbox/codespaces projects show a
 * colored dot + label so it is always visible where commands execute.
 */
function TerminalEnvironmentBadge() {
  const language = useLanguage()
  const layout = useLayout()
  const sdk = useSDK()
  const project = createMemo(() => {
    const key = pathKey(sdk().directory)
    return layout.projects.list().find((item) => pathKey(item.worktree) === key)
  })
  const kind = createMemo(() => {
    const item = project()
    return item ? resolveInitialEnvironment(item).kind : ("local" as const)
  })
  ...
  const dot = createMemo(() => (kind() === "codespaces" ? "bg-violet-400" : "bg-sky-400"))
  return (
    <Show when={kind() !== "local"}>
      <div class="h-full flex items-center px-2" title={language.t("session.new.environment.change")}>
        <span class={`size-1.5 shrink-0 rounded-full ${dot()}`} aria-hidden="true" />
        <span class="ml-1.5 text-[12px] font-[440] leading-5 text-v2-text-text-faint">{label()}</span>
      </div>
    </Show>
  )
}
