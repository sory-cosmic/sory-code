...
import * as IntegratedBrowser from "./integrated-browser"
...
export function registerIpcHandlers(deps: Deps) {
  ...
  const browserWindow = (event: IpcMainInvokeEvent) => {
  ...
  ipcMain.handle("integrated-browser-open", (event: IpcMainInvokeEvent, url?: string) => {
    const win = browserWindow(event)
    IntegratedBrowser.open(win, url)
    return IntegratedBrowser.current(win)
  })
  ipcMain.handle("integrated-browser-close", (event: IpcMainInvokeEvent) => IntegratedBrowser.close(browserWindow(event)))
  ipcMain.handle("integrated-browser-navigate", (event: IpcMainInvokeEvent, url: string) =>
    IntegratedBrowser.navigate(browserWindow(event), url),
  )
  ipcMain.handle("integrated-browser-back", (event: IpcMainInvokeEvent) => IntegratedBrowser.back(browserWindow(event)))
  ipcMain.handle("integrated-browser-forward", (event: IpcMainInvokeEvent) =>
    IntegratedBrowser.forward(browserWindow(event)),
  )
  ipcMain.handle("integrated-browser-reload", (event: IpcMainInvokeEvent) => IntegratedBrowser.reload(browserWindow(event)))
  ipcMain.handle(
    "integrated-browser-bounds",
    (event: IpcMainInvokeEvent, bounds: { x: number; y: number; width: number; height: number }) =>
      IntegratedBrowser.bounds(browserWindow(event), bounds),
  )
