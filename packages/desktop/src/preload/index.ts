...
import type { ElectronAPI, IntegratedBrowserState, WslServersEvent } from "./types"
...
const api: ElectronAPI = {
  ...
  browser: {
    open: (url) => ipcRenderer.invoke("integrated-browser-open", url),
    close: () => ipcRenderer.invoke("integrated-browser-close"),
    navigate: (url) => ipcRenderer.invoke("integrated-browser-navigate", url),
    back: () => ipcRenderer.invoke("integrated-browser-back"),
    forward: () => ipcRenderer.invoke("integrated-browser-forward"),
    reload: () => ipcRenderer.invoke("integrated-browser-reload"),
    setBounds: (bounds) => ipcRenderer.invoke("integrated-browser-bounds", bounds),
    subscribe: (cb) => {
      const handler = (_: unknown, state: IntegratedBrowserState) => cb(state)
      ipcRenderer.on("integrated-browser-state", handler)
      return () => ipcRenderer.removeListener("integrated-browser-state", handler)
    },
  },
}
