...
import { EnvironmentAdapterType } from "@/control-plane/adapters/environment-adapter"
import { targetFor } from "@/control-plane/environment"
...
function resolveProjectTarget(projectID: ProjectV2.ID, workspaceID?: WorkspaceV2.ID): Effect.Effect<Target> {
  return targetFor(projectID, workspaceID).pipe(
    Effect.catchAll((error) => {
      const message = error instanceof Error ? error.message : String(error)
      return Effect.fail(
        new Error(`Project ${projectID} has no provisionable runtime: ${message}`),
      ) as unknown as Effect.Effect<Target>
    }),
    Effect.orElseSucceed(() => ({ type: "local" as const, directory: process.cwd() })),
  )
}
...
): Effect.Effect<RequestPlan, never, Workspace.Service | Project.Service> {
  return Effect.gen(function* () {
    ...
    // Authoritative path: the active project's environment decides the
    // target. Workspace adapter is `environment` so remote execution
    // (sandbox/codespaces) routes through the existing proxy.
    if (!envWorkspaceID && session?.projectID) {
      const target = yield* resolveProjectTarget(session.projectID, workspaceID)
      if (target.type === "remote" && !shouldStayOnControlPlane(request, url)) {
        const synthetic: Workspace.Info = workspace ?? {
          id: (workspaceID ?? ("default" as WorkspaceV2.ID)),
          type: EnvironmentAdapterType,
        ...
        return RequestPlan.Remote({ request, workspace: synthetic, target, url })
      }
      if (target.type === "local") {
        return RequestPlan.Local({ directory: target.directory, workspaceID: envWorkspaceID ?? workspaceID })
      }
    }
