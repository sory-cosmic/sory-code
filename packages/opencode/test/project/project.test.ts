...
describe("Project.update", () => {
  ...
  it.live("should persist an execution environment", () =>
    Effect.gen(function* () {
      const project = yield* Project.Service
      const tmp = yield* tmpdirScoped({ git: true })
      const result = yield* project.fromDirectory(tmp)
      ...
      const environment = {
        kind: "codespaces" as const,
        repository: "sory/example",
        branch: "dev",
        remoteURL: "https://workspace.example.test",
      }
      const updated = yield* project.update({ projectID: result.project.id, environment })

      expect(updated.environment).toEqual(environment)
      expect((yield* project.get(result.project.id))?.environment).toEqual(environment)
    }),
  )
